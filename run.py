...
def train(step_number, last_save):
    ...
    #RSF: Create a tqdm progress bar for the total training steps
    with tqdm(total=info['MAX_STEPS'], desc="Training Progress", unit="step") as pbar:
            ...
            while epoch_frame < info['EVAL_FREQUENCY']:
                ...
                while not terminal:
                    ...
                    if step_number % info['LEARN_EVERY_STEPS'] == 0 and step_number > info['MIN_HISTORY_TO_LEARN']:
                        ...
                        # RSF: print the loss and q value to the progress bar
                        if step_number % int(10e3) == 0:
                            print(f"Step: {step_number}, Avg Reward (last 30): {perf['avg_rewards'][-1] if len(perf['avg_rewards']) > 0 else 0:.2f}, Loss: {np.mean(ptloss_list):.4f}")
                ...
                perf['avg_rewards'].append(np.mean(perf['episode_reward'][-30:])) # RSF: changed to last 30
...
if __name__ == '__main__':
    ...
    info = {
        "GAME":'roms/ms_pacman.bin', # REZA: gamge name changed here
        "DEVICE":device, #cpu vs gpu set by argument
        "NAME":'reza_pacman', # start files with name
        ...
        "MAX_STEPS":int(50e6), # 50e6 steps is 200e6 frames > e6 is million
    ...
    for key, value in info.items():
        ...
        if isinstance(value, (int, float)) and value > 1e3:
            formatted_value = f"{value:,.0f}"  # Format number with commas and no decimals
        else:
